...
def write_temp_slurm_only_config(base_config_path: Path, provider_id: str) -> Path:
    ...
    tmp_root = Path(tempfile.mkdtemp(prefix="codeswarm-slurm-only-"))
...
def main():
    parser = argparse.ArgumentParser(description="Run a Slurm-only Claude runtime smoke against a single launch provider.")
    ...
    parser.add_argument("--launch-timeout", type=float, default=3600.0)
    args = parser.parse_args()

    if not str(os.environ.get("ANTHROPIC_API_KEY") or "").strip():
        raise RuntimeError("ANTHROPIC_API_KEY must be set for Slurm Claude smoke")
